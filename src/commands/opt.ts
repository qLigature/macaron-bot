...
export async function execute(interaction: CommandInteraction) {
  if (!interaction.memberPermissions?.has("MANAGE_MESSAGES")) return interaction.reply({content: "Beep Boop, you don't have the \"Manage Messages\" permission needed to run this command."})
  ...
  interaction.reply({content: `Would you are currently **${txtOpt(guild.opt)}** of global emojis, would you like to change that?`, components: [row], ephemeral: true})
...
collector.on('collect', async i => {
    found = true;
	if (i.customId == "Yes") {
        await updateGuild(interaction.guildId!, {opt: !guild.opt})
        await i.update({content: `You have been changed to be **${txtOpt(!guild.opt)}**`, components: []})
    } else {
        await interaction.editReply({content: "No change has been made", components: []})
    }
});
