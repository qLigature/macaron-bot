...
export async function execute(interaction: CommandInteraction) {
  ...
  const serverEmojis = Array.from(getMap().values())
    .filter(
      (e) =>
        !g.blacklist.includes(`:${e.name}:`) &&
  ...
  if (serverEmojis.length == 0)
    return interaction.reply({
      content: 'All available emojis are in the blacklist',
      ephemeral: true,
    });
  ...
  interaction.reply({
    content:
      'Please select an emoji you would like to blacklist from being used globally',
    components: [row],
    ephemeral: true,
  });
  ...
  collector.once('collect', async (e: SelectMenuInteraction) => {
    ...
    if (g.blacklist.includes(`:${emoji?.name}:`)) {
      e.update({
        content: `${emoji} **:${emoji?.name}:** is already blacklisted`,
        components: [],
      });
      collector.stop();
      return;
    ...
    e.update({
      content: `${emoji} Are you sure you would like to blacklist **:${emoji?.name}:**?`,
    ...
    collector2.once('collect', async (i) => {
      ...
      if (i.customId == 'Yes') {
        await updateGuild(interaction.guildId!, {
          $push: { blacklist: `:${emoji?.name}:` },
        });
        await i.update({
          content: `You have blacklisted ${emoji} **:${emoji?.name}:**`,
          components: [],
        });
      } else {
