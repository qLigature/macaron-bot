...
export async function execute(interaction: CommandInteraction) {
  ...
  interaction.reply({
    content:
      'Please select an emoji you would like to whitelist from being used globally',
    components: [row],
    ephemeral: true,
  });
  ...
  collector.once('collect', async (e: SelectMenuInteraction) => {
    ...
    e.update({
      content: `${emoji} Are you sure you would like to remove **:${emoji?.name}:** from the blacklist?`,
    ...
    collector2.once('collect', async (i) => {
      ...
      if (i.customId == 'Yes') {
        await updateGuild(interaction.guildId!, {
          $pull: { blacklist: `:${emoji?.name}:` },
        });
        await i.update({
          content: `You have removed ${emoji} **:${emoji?.name}:** from the blacklist`,
          components: [],
        });
      } else {
